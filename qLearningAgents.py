...
class HierarchicalDDPGAgent(Agent):
    def __init__(self, **args):
        ...
        # self.min_epsilon = 0.01
        # self.decay = .999
        self.speedAgent = DqnModule(nb_features = self.nb_speedFeatures, featureExtractor = simpleExtractor)
    ...
    def getAction(self, state):
        ...
        speedQValues = self.speedAgent.getQValues(state)
        collisionQValues = self.collisionAgent.getQValues(state)
        scalarizedQValues = scaleParameters[0] * speedQValues + scaleParameters[1] * collisionQValues
    ...
    def update(self, state, action, nextState, reward, done):
        ...
        self.arbitrator.update(state, self.arbitratorAction, nextState, float(reward), done)
        self.speedAgent.update(state, action, nextState, reward, done)
        self.collisionAgent.update(state, action, nextState, reward, done)
